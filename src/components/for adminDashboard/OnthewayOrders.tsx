      ...
      <div className="overflow-hidden">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {orders.map((order) => (
              <tr key={order.productname} className="hover:bg-gray-100">
                ...
                <td className="px-6 py-4 text-right text-sm text-gray-700">{order.isreview ? 'Yes' : 'No'}</td>
                <td className="px-6 py-4 text-right text-sm font-semibold flex justify-center items-center">
                  <Image src="/delete.png" height={25} width={25} alt="Delete Order" />
                </td>
              </tr>
            ))}
          </tbody>
