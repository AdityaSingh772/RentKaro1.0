// // import React from 'react'

// // const Firstpage = () => {
// //   return (
// //     <div className='bg-[#27374D] h-screen'>
// //       <div className='h-1/2'>
      
// //       <div className='border-b border-white flex   flex-col p-9'>
// //         <p className='text-yellow-400 text-[6rem] mt-9'>RentKaro </p>
// //         <span className='text-white  text-3xl mb-9 mt-5'> You can rent </span>
// //         <p className='mt-3'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Reprehenderit numquam fugit sequi reiciendis. Expedita quos esse odit aut obcaecati alias architecto eos ipsam fugit amet earum fuga fugiat, dicta ipsum!</p>
// //       </div>
// //       </div>
// //     </div>
// //   )
// // }

// // export default Firstpage

// // import React from 'react';
// // import './firstpage.css';

// // const Firstpage = () => {
// //   return (
// //     <>
// //     <div className=".custom-shape-divider-top-1716489011">
// //     <svg data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
// //         <path d="M0,0V46.29c47.79,22.2,103.59,32.17,158,28,70.36-5.37,136.33-33.31,206.8-37.5C438.64,32.43,512.34,53.67,583,72.05c69.27,18,138.3,24.88,209.4,13.08,36.15-6,69.85-17.84,104.45-29.34C989.49,25,1113-14.29,1200,52.47V0Z" opacity=".25" className="shape-fill"></path>
// //         <path d="M0,0V15.81C13,36.92,27.64,56.86,47.69,72.05,99.41,111.27,165,111,224.58,91.58c31.15-10.15,60.09-26.07,89.67-39.8,40.92-19,84.73-46,130.83-49.67,36.26-2.85,70.9,9.42,98.6,31.56,31.77,25.39,62.32,62,103.63,73,40.44,10.79,81.35-6.69,119.13-24.28s75.16-39,116.92-43.05c59.73-5.85,113.28,22.88,168.9,38.84,30.2,8.66,59,6.17,87.09-7.5,22.43-10.89,48-26.93,60.65-49.24V0Z" opacity=".5" className="shape-fill"></path>
// //         <path d="M0,0V5.63C149.93,59,314.09,71.32,475.83,42.57c43-7.64,84.23-20.12,127.61-26.46,59-8.63,112.48,12.24,165.56,35.4C827.93,77.22,886,95.24,951.2,90c86.53-7,172.46-45.71,248.8-84.81V0Z" className="shape-fill"></path>
// //     </svg>
// // </div>
// //     <div className='bg-[#27374D] h-screen '>
// //       <div className='max-w-4xl w-full p-9'>
// //         <div className='border-b border-white flex flex-col'>
// //           <p className='text-yellow-400 text-[6rem] mt-9'>RentKaro</p>
// //           <span className='text-white text-3xl mb-9 mt-5'>You can rent</span>
// //           <p className='mt-3 text-white'>
// //             Lorem ipsum dolor sit amet consectetur adipisicing elit. Reprehenderit numquam fugit sequi reiciendis. Expedita quos esse odit aut obcaecati alias architecto eos ipsam fugit amet earum fuga fugiat, dicta ipsum!
// //           </p>
// //         </div>
// //       </div>
// //     </div>
// //     </>
// //   );
// // }

// // export default Firstpage;






// "use client"
// import React from 'react';
// import './utils/firstpage.css'; // Import the custom CSS file
// import { useUser } from '@auth0/nextjs-auth0/client';
// import Link from 'next/link';

// const Firstpage = () => {
//   const {user,error,isLoading}=useUser()
//   return (
//     <div className="relative bg-[#27374D] min-h-screen flex flex-col justify-between">
//       <div className="max-w-4xl w-full p-9">
//         <div className="custom-shape-divider-top-1716489011">
//           <svg
//             data-name="Layer 1"
//             xmlns="http://www.w3.org/2000/svg"
//             viewBox="0 0 1200 120"
//             preserveAspectRatio="none"
//           >
//             <path
//               d="M0,0V46.29c47.79,22.2,103.59,32.17,158,28,70.36-5.37,136.33-33.31,206.8-37.5C438.64,32.43,512.34,53.67,583,72.05c69.27,18,138.3,24.88,209.4,13.08,36.15-6,69.85-17.84,104.45-29.34C989.49,25,1113-14.29,1200,52.47V0Z"
//               opacity=".25"
//               className="shape-fill"
//             ></path>
//             <path
//               d="M0,0V15.81C13,36.92,27.64,56.86,47.69,72.05,99.41,111.27,165,111,224.58,91.58c31.15-10.15,60.09-26.07,89.67-39.8,40.92-19,84.73-46,130.83-49.67,36.26-2.85,70.9,9.42,98.6,31.56,31.77,25.39,62.32,62,103.63,73,40.44,10.79,81.35-6.69,119.13-24.28s75.16-39,116.92-43.05c59.73-5.85,113.28,22.88,168.9,38.84,30.2,8.66,59,6.17,87.09-7.5,22.43-10.89,48-26.93,60.65-49.24V0Z"
//               opacity=".5"
//               className="shape-fill"
//             ></path>
//             <path
//               d="M0,0V5.63C149.93,59,314.09,71.32,475.83,42.57c43-7.64,84.23-20.12,127.61-26.46,59-8.63,112.48,12.24,165.56,35.4C827.93,77.22,886,95.24,951.2,90c86.53-7,172.46-45.71,248.8-84.81V0Z"
//               className="shape-fill"
//             ></path>
//           </svg>
//         </div>
//         <div className="border-b border-white flex flex-col">
//           <div className="rentkaro-container">
//             <p className="text-yellow-400 text-[6rem] mt-9 text-effect py-5">RentKaro</p>
//           </div>
//           <div className='wrapper text-2xl'>
//           <p>You can either</p>
//           <div className='words text-2xl'>
//              <span className="animate">BUY</span>
//              <span className="animate">SELL</span>
//              <span className="animate">RENT</span>
//           </div>
//           </div>
          
//           <p className="mt-3 text-white">
//             Lorem ipsum dolor sit amet consectetur adipisicing elit. Reprehenderit numquam fugit sequi reiciendis. Expedita quos esse odit aut obcaecati alias architecto eos ipsam fugit amet earum fuga fugiat, dicta ipsum!
//           </p>
//         </div>
//       </div>
//       <div className="flex justify-evenly items-center bg-[#27374D] py-4 h-[300px]">
//         <button className="text-white bg-blue-500 hover:bg-blue-600 py-4 px-10 rounded-lg mr-4">
//           <Link href="/api/auth/login">
//            Login
//           </Link>
//           </button>
//         <button className="text-white bg-green-500 hover:bg-green-600 py-4 px-10 rounded-lg ml-4">
//         <Link href="/api/auth/login">
//            Sign up
//           </Link>
//           </button>
//       </div>
//     </div>

//     // <div className="flex justify-evenly items-center bg-[#27374D] py-4 h-[300px]">
//     //     <button className="text-white bg-blue-500 hover:bg-blue-600 py-4 px-10 rounded-lg mr-4">
//     //       <Link href="/api/auth/login">
//     //        Login
//     //       </Link>
//     //       </button>
//     //     <button className="text-white bg-green-500 hover:bg-green-600 py-4 px-10 rounded-lg ml-4">
//     //       Signup
//     //       </button>
//     //   </div>
//   );
// }

// export default Firstpage;




"use client";

import { motion } from "framer-motion";
import React from "react";
import { AuroraBackground } from "./ui/aurora-background";
import Link from "next/link";
import { FlipWordsDemo } from "./FlipWordsDemo";


export function Firstpage() {
  return (
    <AuroraBackground>
      <motion.div
        initial={{ opacity: 0.0, y: 40 }}
        whileInView={{ opacity: 1, y: 0 }}
        transition={{
          delay: 0.3,
          duration: 0.8,
          ease: "easeInOut",
        }}
        className="w-screen h-screen relative flex flex-col gap-4 items-center justify-center px-4"
      >
      
        <FlipWordsDemo/>
        <div className="flex justify-evenly items-center gap-x-[2rem] h-[300px] px-0">
        <button className=" block w-full px-10 border-b-2 border-blue-500 py-5 hover:bg-white bg-black text-white hover:text-blue-500 rounded-full  shadow-md hover:shadow-lg">
          <Link href="/api/auth/login">
            <span className="font-bold">user Login</span>
          </Link>
          </button>
          <button className="btn text-white shadow-md shadow-[#EAECC6] hover:text-black py-4 px-10 rounded-md mr-4">
          <Link href="/api/auth/login">
            <span className="font-bold">Signup</span>
           
          </Link>
          </button>
      </div>
      </motion.div>
    </AuroraBackground>
  );
}
